...
public class ApiService
{
    ...
    public async Task<bool> CreateTask(string tittle, string description, string status, string token)
    {
    ...
    public async Task<bool> UpdateTask(TaskDto task, string token)
    {
        _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
        var response = await _httpClient.PutAsJsonAsync($"{_configuration["ApiBaseUrl"]}/tasks/{task.Id}", task);
